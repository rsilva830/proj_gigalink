...
class TransportadoraAdmModel extends MainModel {
    ...
    public function listar_transportadoras() {
        ...
        $query = $this->db->query('SELECT * FROM transportadora ' . $where . ' ORDER BY id DESC' . $query_limit, $id);
        return $query->fetchAll();
    }

    public function obtem_transportadora() {
        ...
        if ('POST' == $_SERVER['REQUEST_METHOD'] && !empty($_POST['insere_transportadora'])) {

            // Remove o campo insere_fornecedor para não gerar problema com o PDO
            unset($_POST['insere_transportadora']);

            // Atualiza os dados
            $query = $this->db->update('transportadora', 'id', $id, $_POST);
            if ($query) {
                $this->form_msg = '<p class="success">Transportadora atualizada com sucesso!</p>';
            }
        }
        $query = $this->db->query('SELECT * FROM transportadora WHERE id = ? LIMIT 1', array($id));
    ...
    public function insere_transportadora() {
        if ('POST' != $_SERVER['REQUEST_METHOD'] || empty($_POST['insere_transportadora'])) {
            return;
        }
        if (chk_array($this->parametros, 0) == 'edit') {
        ...
        $query = $this->db->insert('transportadora', $_POST);
    ...
    public function apaga_transportadora() {
        ...
        if (chk_array($this->parametros, 2) != 'confirma') {
            $mensagem = '<p class="alert">Tem certeza que deseja apgar a transportadora?</p>';
            $mensagem .= '<p><a href="' . $_SERVER['REQUEST_URI'] . '/confirma/">Sim</a> | ';
            $mensagem .= '<a href="' . HOME_URI . '/transportadoras/adm/">Não</a></p>';
            return $mensagem;
        }

        $id = (int) chk_array($this->parametros, 1);
        $query = $this->db->delete('transportadora', 'id', $id);
        echo '<meta http-equiv="Refresh" content="0; url=' . HOME_URI . '/transportadoras/adm/">';
        echo '<script type="text/javascript">window.location.href = "' . HOME_URI . '/transportadoras/adm/";</script>';
    }
    ...
    public function paginacao() {
        ...
        $query = $this->db->query('SELECT COUNT(*) as total FROM transportadora ');
        $linhas = $query->fetch();
        $linhas = $linhas['total'];
        $resPorPg = $this->posts_por_pagina;
        $qtPg = (($linhas % $resPorPg) > 0) ? (int) ($linhas / $resPorPg) + 1 : ($linhas / $resPorPg);
        //seta o caminho
        $caminho = HOME_URI . '/transportadoras/index/page/';
        //imprime a paginacao
        if ($qtPg > 1) {
            $prox = $pPg + 1;
            $ant = $pPg - 1;

...
class TelefoneAdmModel extends MainModel {
    ...
    public function listar_telefones() {
        ...
        $query = $this->db->query('SELECT * from telefone' . $where . ' ORDER BY id_fornecedor DESC' . $query_limit, $id);
        return $query->fetchAll();
    }

    //SELECT t.ddd,t.numero,f.nome FROM telefone t,fornecedor f where t.id_fornecedor = f.id order by f.id;
    ...
    public function obtem_telefone() {
        ...
        if ('POST' == $_SERVER['REQUEST_METHOD'] && !empty($_POST['insere_telefone'])) {

            // Remove o campo insere_fornecedor para não gerar problema com o PDO
            unset($_POST['insere_telefone']);

            // Atualiza os dados
            $query = $this->db->update('telefone', 'id', $id, $_POST);
            if ($query) {
                $this->form_msg = '<p class="success">Telefone atualizado com sucesso!</p>';
            }
        }
        $query = $this->db->query('SELECT * FROM telefone WHERE id = ? LIMIT 1', array($id));
    ...
    public function insere_telefone() {
        if ('POST' != $_SERVER['REQUEST_METHOD'] || empty($_POST['insere_telefone'])) {
            return;
        }
        if (chk_array($this->parametros, 0) == 'edit') {
        ...
        $query = $this->db->insert('telefone', $_POST);
    ...
    public function apaga_telefone() {
        ...
        if (chk_array($this->parametros, 2) != 'confirma') {
            $mensagem = '<p class="alert">Tem certeza que deseja apgar o telefone?</p>';
            $mensagem .= '<p><a href="' . $_SERVER['REQUEST_URI'] . '/confirma/">Sim</a> | ';
            $mensagem .= '<a href="' . HOME_URI . '/telefone/adm/">Não</a></p>';
            return $mensagem;
        }

        $id = (int) chk_array($this->parametros, 1);
        $query = $this->db->delete('telefone', 'id', $id);
        echo '<meta http-equiv="Refresh" content="0; url=' . HOME_URI . '/telefones/adm/">';
        echo '<script type="text/javascript">window.location.href = "' . HOME_URI . '/telefones/adm/";</script>';
    }
    ...
    public function paginacao() {
        ...
        $query = $this->db->query('SELECT COUNT(*) as total FROM telefone ');
        $linhas = $query->fetch();
        $linhas = $linhas['total'];
        $resPorPg = $this->posts_por_pagina;
        $qtPg = (($linhas % $resPorPg) > 0) ? (int) ($linhas / $resPorPg) + 1 : ($linhas / $resPorPg);
        //seta o caminho
        $caminho = HOME_URI . '/telefones/index/page/';
        //imprime a paginacao
        if ($qtPg > 1) {
            $prox = $pPg + 1;
            $ant = $pPg - 1;
